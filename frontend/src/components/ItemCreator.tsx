...
import { Package, Upload, Sword, Shield, FlaskConical, Coins, Gem } from 'lucide-react';
    ...
    <div className="bg-[#0f0f13] border border-white/10 rounded-xl p-5 shadow-2xl relative overflow-hidden font-sans">
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-4 relative z-10">
        <h3 className="text-amber-500 font-bold uppercase tracking-[0.15em] text-xs flex items-center gap-2">
          {mode === 'forge' ? <Package size={14} /> : <Coins size={14} />} 
          {mode === 'forge' ? 'Forjar Item' : 'Criar Tesouro'}
        </h3>
      ...
      <div className="flex border-b border-white/10 mb-5 relative z-10">
        <button type="button" onClick={() => setMode('forge')} className={`flex-1 pb-2 text-[10px] uppercase font-bold tracking-widest transition-colors flex items-center justify-center gap-1.5 ${mode === 'forge' ? 'text-amber-500 border-b-2 border-amber-500' : 'text-gray-500 hover:text-gray-300'}`}><Sword size={12}/> Equipamento</button>
        <button type="button" onClick={() => setMode('treasure')} className={`flex-1 pb-2 text-[10px] uppercase font-bold tracking-widest transition-colors flex items-center justify-center gap-1.5 ${mode === 'treasure' ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-gray-500 hover:text-gray-300'}`}><Gem size={12}/> Ouro & Riquezas</button>
      </div>
